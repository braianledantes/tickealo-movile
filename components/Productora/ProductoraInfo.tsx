...
export function ProductoraInfo({ evento, visible, onClose }: Props) {
  const { getEventosByProductora, getProductora } = useProductora();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await getProductora(evento.productora.userId);
        setProductora(data || null);
        const eventos = await getEventosByProductora(evento.productora.userId);
        setEventosCount(eventos?.length ?? 0);
      } catch (error) {
        console.error("Error al obtener info de la productora:", error);
        setEventosCount(0);
      } finally {
        setLoading(false);
      }
  ...
  }, [visible, evento.productora.userId]);
